...
const services=[
  ...
  {name:'هتل',icon:'⌂',vertical:'hotel',origin:false,description:'اقامت و رزرو هتل'},
  {name:'ویلا',icon:'⌂',vertical:'vacation_rental',origin:false,description:'ویلا، سوئیت و اقامتگاه'},
  {name:'تور',icon:'◉',vertical:'tour',origin:true,description:'تورهای داخلی و خارجی'},
  {name:'قطار',icon:'▣',vertical:'train',origin:true,description:'سفر ریلی و ایستگاه‌ها'},
  {name:'خودرو',icon:'▱',vertical:'car_rental',origin:true,description:'اجاره خودرو در مقصد'},
  {name:'کروز',icon:'≋',vertical:'cruise',origin:false,description:'سفر دریایی و کشتی'},
  {name:'ویزا',icon:'▤',vertical:'visa',origin:true,description:'بررسی و درخواست ویزا'},
] as const;
...
export function HomeSearchExperience(){
  ...
  const destinationLabel=service.vertical==='hotel'||service.vertical==='vacation_rental'?'مقصد، هتل یا منطقه':service.vertical==='visa'?'کشور مقصد':service.vertical==='car_rental'?'محل بازگشت':service.vertical==='cruise'?'مسیر / مقصد':'مقصد';
  const dateLabel=service.vertical==='visa'?'تاریخ سفر':service.vertical==='cruise'?'تاریخ حرکت':service.vertical==='hotel'||service.vertical==='vacation_rental'?'ورود و خروج':'تاریخ سفر';
  return <section className="search-wrap" id="search" aria-label="جست‌وجوی یکپارچه سفر" onKeyDown={event=>{if(event.key==='Escape')setPopover(null)}}>
    <div className="search-heading"><div><span>جست‌وجوی هوشمند سفر</span><b>{service.name}</b><small>{service.description}</small></div><em>قیمت و موجودی از منبع معتبر</em></div>
