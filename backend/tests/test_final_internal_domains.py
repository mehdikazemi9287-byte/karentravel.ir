...
def test_invoice_is_backend_derived_idempotent_and_owner_scoped(client):
    ...
    headers = {**auth(backoffice["access_token"]), "Idempotency-Key": "invoice-final-domain"}
...
def test_support_case_thread_distinguishes_human_and_blocks_cross_tenant(client):
    ...
    assert client.post("/support/cases", headers={**auth(faraz["access_token"]), "Idempotency-Key": "case-cross-tenant"}, json=payload).status_code == 404
    opened = client.post("/support/cases", headers={**auth(employee["access_token"]), "Idempotency-Key": "case-owner-final"}, json=payload)
...
def test_installment_execution_and_settlement_posting_are_tenant_safe(client):
    ...
    with SessionLocal() as db:
        ...
        check = PriceCheck(tenant_id=owner.tenant_id, offer_id=offer.id, user_id=owner.id, command_id="final-check", units=1, amount=3_000_000, currency="IRR", snapshot_json='{"total_amount":3000000,"currency":"IRR"}', snapshot_hash="final-snapshot", expires_at=datetime.now(timezone.utc)+timedelta(minutes=10)); db.add(check); db.flush()
        ...
        payment = PaymentIntent(tenant_id=owner.tenant_id, reservation_id=reservation.id, user_id=owner.id, command_id="captured-final", amount=3_000_000, status="captured", captured_amount=3_000_000, provider_reference="sandbox-evidence"); db.add(payment); db.commit(); plan_id = plan.id; reservation_id = reservation.id
    requested = client.post(f"/reservations/{reservation_id}/installments?plan_id={plan_id}", headers={**auth(employee["access_token"]), "Idempotency-Key": "installment-final"})
    ...
    settlement_headers = {**auth(finance["access_token"]), "Idempotency-Key": "settlement-final"}
...
def test_admin_role_workflow_prevents_self_and_privilege_escalation(client):
    ...
    with SessionLocal() as db:
        admin = db.scalar(select(User).where(User.email == "tenant-admin@aftab.test")); target = User(tenant_id=admin.tenant_id, email="role-target@aftab.test", name="کاربر نقش", role="customer"); db.add(target); db.commit(); target_id = target.id; admin_id = admin.id
