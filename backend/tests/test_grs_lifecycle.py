...
from app.grs_lifecycle import (
    GRSLifecycleError, reserve_with_grs, book_with_grs,
...
def _make_tenant_user_reservation():
    with SessionLocal() as db:
        ...
        user = User(tenant_id=tenant.id, email=f"grs-test-{uuid.uuid4().hex[:8]}@example.test", name="[TEST] Guest", role="customer")
        db.add(user)
        db.flush()
...
def test_reserve_success_transitions_to_reserved_and_stores_confirmation_code():
    tenant_id, reservation_id = _make_tenant_user_reservation()
    ...
    with SessionLocal() as db:
        ...
        outcome = reserve_with_grs(db, reservation=reservation, adapter=adapter, reserve_payload=RESERVE_PAYLOAD, command_id="cmd-1")
        db.commit()
...
def test_reserve_business_failure_marks_reservation_failed():
    ...
    with SessionLocal() as db:
        set_tenant_context(db, tenant_id)
        reservation = db.get(Reservation, reservation_id)
        outcome = reserve_with_grs(db, reservation=reservation, adapter=adapter, reserve_payload=RESERVE_PAYLOAD, command_id="cmd-2")
        db.commit()
...
def test_reserve_ambiguous_timeout_opens_reconciliation_case_not_a_guess():
    ...
    with SessionLocal() as db:
        set_tenant_context(db, tenant_id)
        reservation = db.get(Reservation, reservation_id)
        outcome = reserve_with_grs(db, reservation=reservation, adapter=adapter, reserve_payload=RESERVE_PAYLOAD, command_id="cmd-3")
        db.commit()
        assert outcome.ok is False
...
def test_reserve_is_idempotent_for_same_command_id():
    ...
    with SessionLocal() as db:
        set_tenant_context(db, tenant_id)
        reservation = db.get(Reservation, reservation_id)
        reserve_with_grs(db, reservation=reservation, adapter=adapter, reserve_payload=RESERVE_PAYLOAD, command_id="cmd-5")
        db.commit()
    ...
    with SessionLocal() as db:
        set_tenant_context(db, tenant_id)
        reservation = db.get(Reservation, reservation_id)
        # the fake transport has zero queued responses left - if reserve_with_grs
        # called it again this would raise IndexError, proving no duplicate call
        outcome = reserve_with_grs(db, reservation=reservation, adapter=adapter, reserve_payload=RESERVE_PAYLOAD, command_id="cmd-5")
...
def test_book_refuses_without_independently_confirmed_payment():
    ...
    with SessionLocal() as db:
        ...
        with pytest.raises(GRSLifecycleError, match="payment/credit confirmation was not proven"):
            book_with_grs(db, reservation=reservation, adapter=adapter, command_id="cmd-6", payment_or_credit_confirmed=False)
...
def test_book_succeeds_only_with_confirmed_payment_and_booked_status():
    ...
    with SessionLocal() as db:
        ...
        db.commit()
        outcome = book_with_grs(db, reservation=reservation, adapter=adapter, command_id="cmd-7", payment_or_credit_confirmed=True)
        db.commit()
        assert outcome.ok is True
...
def test_book_with_unexpected_status_after_success_opens_reconciliation_not_a_guess():
    ...
    with SessionLocal() as db:
        ...
        db.commit()
        outcome = book_with_grs(db, reservation=reservation, adapter=adapter, command_id="cmd-8", payment_or_credit_confirmed=True)
        db.commit()
...
def test_cancellation_lifecycle_two_step_not_immediately_final():
    ...
    with SessionLocal() as db:
        ...
        request_cancellation_with_grs(db, reservation=reservation, adapter=adapter, command_id="cmd-9a")
        db.commit()
        assert reservation.status == "cancel_requested"  # not yet final

        accept_cancellation_with_grs(db, reservation=reservation, adapter=adapter, command_id="cmd-9b")
        db.commit()
