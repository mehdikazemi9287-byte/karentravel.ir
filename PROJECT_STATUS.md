...
## سفر رزرو ← پرداخت ← مالی: از نمایشی به واقعی — ۲۰۲۶-۰۸-۲۳

- اعتبار سازمانی (Organization Credit) که مدلش موجود بود ولی هیچ endpoint‌ای آن را صدا نمی‌زد، اکنون کاملاً واقعی است: ledger واقعی، سقف اعتبار، رزرو/برداشت اتمیک، ایزوله بین tenant، تست شده روی سایت واقعی.
- باگ مالی واقعی پیدا و رفع شد: مبلغ رزروشده کیف‌پول هرگز capture/release نمی‌شد و برای همیشه "reserved" می‌ماند؛ اکنون در تکمیل رزرو capture و در کنسلی release می‌شود.
- رویدادهای پرداخت اکنون در Timeline واقعی سفر نمایش داده می‌شوند؛ صورتحساب اکنون خودکار در لحظه تکمیل رزرو صادر می‌شود.
- بدون اعتبار واقعی درگاه (ZarinPal)، اتصال واقعی درگاه همچنان EXTERNAL BLOCKER صادقانه است — هیچ موفقیت جعلی نمایش داده نمی‌شود.
- HEAD و migration head در CHECKPOINT.md ثبت شده. تست: ۹۲ موفق، ۶ skip.

## سفر پس از رزرو: از نمایشی به واقعی — ۲۰۲۶-۰۸-۲۳

- سفرهای من، جزئیات سفر، Timeline، تغییرات سفر، کنسلی/استرداد، پشتیبانی انسانی و اعلان‌ها اکنون همگی از داده واقعی backend تغذیه می‌شوند؛ هیچ رزرو/تغییر/استرداد جعلی ساخته نشد. تأیید نهایی روی سایت واقعی `http://95.38.184.209/` با رزرو واقعی، دسکتاپ و موبایل.
- دو باگ زیرساختی واقعی پیدا و رفع شد: nginx به کانتینر preview قدیمی (پورت ۳۱۰۰) اشاره می‌کرد نه کانتینر مدیریت‌شده با docker-compose (پورت ۳۰۰۰)؛ و API در حال اجرا نسخه قدیمی کد را داشت (rebuild فراموش‌شده). هر دو با backup و تأیید صریح کاربر رفع شدند.
- HEAD: `1114634e0ff7c0edec824a91c752053a7b883b3a`؛ migration head: `20260823_18`. تست: ۹۰ موفق، ۶ skip.
